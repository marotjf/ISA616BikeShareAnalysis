...
Within this document I will detail the process of how I developed two different models to help DownTown BikeSharing LLC. predict bike rentals on a daily basis. After I summarize the data and detail the pre-processing steps, I will delve into the models built, the output, and conclusions.

The first part of the analysis includes multiple regression analysis. Due to maintenance costs and risk mitigation, it is advantageous for Downtown BikeSharing to better the predict the necessary amount of bikes to make available each day. Additionally, Downtown BikeSharing's marketing team is attempting to transform more casual riders into registered riders due to the high profitability and retention rate within the membership program. In order to do so, they have increased the marketing budget in terms of advertising towards casual riders. To assist in this effort, I developed a regression tree analysis using array of variables within the data set. This will allow for the marketing team to understand which days attract the most casual riders and to factor this into their marketing plan. I will provide more detail later in the analysis regarding the results of these analyses and their potential effects on DownTown BikeShare. 


### Package Management and Loading Data
The below code includes package management using pacman. If packman is necessary, it will load all the required packages to their version when used before a possible package update. 
...
*	**count:** number of total rentals (Note that casual+registered=count, so you shouldn’t use casual and registered to predict count. I have included the breakdown in case you wish to predict casual and registered with separate models.) Numerical
...
### Predicting Count of Registered Rentals
